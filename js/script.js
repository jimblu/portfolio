...
const RevealOnScroll = (element, heightElement, effectElement) => {
  window.addEventListener("scroll", () => {
    if (
      scrollY - heightElement >=
      element.getBoundingClientRect().top - window.innerHeight + scrollY
    ) {
...
window.addEventListener("scroll", () => {
  container.style.backgroundPositionX = 1000 + window.scrollY / 7 + "px";
  projects.style.backgroundPositionX = window.scrollY / 7 + "px";
  console.log("yeees");
});
